...
## ADR-010 — Billing module: dual invoice numbering + client-direct email

**Decision:** Invoices carry two identifiers: `number` (user-provided from the external accounting system) and `internalRef` (auto-generated by this app as `INT-YYYY-NNN`). Email reminders go directly to `client.email`, not to an internal firm address.

**Why — dual numbering:** The firm's accounting software is the system of record for invoice numbers; this app must not generate numbers that could conflict or duplicate. However, the app needs a stable internal reference for deduplication, linking, and audit. The two fields serve different owners: the accountant owns `number`, the app owns `internalRef`.

**Why — client email:** The goal is automated collection: the client receives the invoice notification and payment reminder directly, removing the manual forwarding step. `client.email` is optional — if null, the send step is skipped silently and logged.
