...
export default function ProjectDetailClient({ project }: { project: Project }) {
  ...
  async function handleRemoveItem(costItemId: string) {
    await fetch(`/api/projects/${project.id}/cost-items?costItemId=${costItemId}`, { method: "DELETE" });
    refresh();
  }

  async function handleRemoveCommitment(commitmentId: string) {
    await fetch(`/api/projects/${project.id}/commitments?commitmentId=${commitmentId}`, { method: "DELETE" });
    refresh();
  }

  async function handleRemovePayment(commitmentId: string, paymentId: string) {
    await fetch(`/api/projects/${project.id}/commitments/${commitmentId}/payments?paymentId=${paymentId}`, { method: "DELETE" });
    refresh();
  }
    ...
    <div className="flex flex-col h-full">
                    ...
                    return (
                      <Fragment key={item.id}>
                        ...
                        {item.commitments.map((c) => {
                          ...
                          const showExpand = c.payments.length > 0 && (c.status !== "Pagado" || c.payments.length > 1);
      ...
      {showAddItem && (
        <AddAPUItemModal
          ...
          onAdded={() => { setShowAddItem(false); refresh(); }}
        />
      )}
      ...
      {addCommitmentTo && (
        <CommitmentForm
          ...
          onCreated={() => { setAddCommitmentTo(null); refresh(); }}
        />
      )}
      ...
      {addPaymentTo && (
        <PaymentForm
          ...
          onCreated={() => { setAddPaymentTo(null); refresh(); }}
        />
      )}
    </div>
  );
